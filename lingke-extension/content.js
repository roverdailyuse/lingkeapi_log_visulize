...
function createMsUI(id, title, filterKey) {
    ...
    selAll.innerText = '全选';
    selAll.onclick = (e) => { e.stopPropagation(); msStates[filterKey].selected = new Set(msStates[filterKey].allOptions); renderMsOpts(id, filterKey); updateChart(); };

    const clearAll = document.createElement('span');
    clearAll.className = 'lk-ms-act-btn';
    clearAll.innerText = '清空';
    clearAll.onclick = (e) => { e.stopPropagation(); msStates[filterKey].selected.clear(); renderMsOpts(id, filterKey); updateChart(); };
...
function renderMsOpts(id, filterKey) {
    ...
    if (st.selected.size === 0 || st.selected.size === st.allOptions.size) hdr.innerText = `全部 ${id.replace('sel-', '')}`;
    ...
    opts.forEach(o => {
        ...
        chk.type = 'checkbox';
        chk.checked = st.selected.size === 0 ? true : st.selected.has(o);
        chk.onchange = (e) => {
            if (st.selected.size === 0 && !e.target.checked) {
                st.selected = new Set(st.allOptions);
                st.selected.delete(o);
            } else if (e.target.checked) st.selected.add(o);
...
function updateMsData(data) {
    ...
    const process = (k) => {
        ...
        if(st.allOptions.size > prevSize) changed = true;
    };
...
function updateChart() {
    ...
    const isSel = (k, v) => msStates[k].selected.size===0 || msStates[k].selected.has(v);
    ...
    if (pieChartInstance) {
        const modelStats = {};
        filtered.forEach(d => modelStats[d.model] = (modelStats[d.model] || 0) + d.cost);
        const pieData = Object.entries(modelStats).map(([name, value]) => ({name, value})).sort((a,b)=>b.value-a.value).slice(0, 10); // 取前10

        pieChartInstance.setOption({
            animation: false,
            tooltip: { trigger: 'item', formatter: '{b}: ${c} ({d}%)' },
            series: [{ type: 'pie', radius: ['40%', '70%'], center: ['50%','50%'], data: pieData, label:{show:false} }]
        });
    }
...
function updateViewStats(filtered) {
    ...
    const sv = opt.dataZoom[0].startValue;
    const ev = opt.dataZoom[0].endValue;

    let sum = 0, reqs = 0, toks = 0;
    filtered.forEach(d => { if(d.ts>=sv && d.ts<=ev){ sum+=d.cost; reqs++; toks+=d.tokens; } });

    const hrs = (ev-sv)/3600000;
    document.getElementById('lk-v-total').innerText = `$${sum.toFixed(5)}`;
    document.getElementById('lk-v-avg-h').innerText = `$${(hrs>0?sum/hrs:0).toFixed(4)}`;
    document.getElementById('lk-v-tokens').innerText = `Tokens: ${toks.toLocaleString()} | Req: ${reqs}`;
}
...
async function initExtensionUI() {
        ...
        <div class="lk-content-layout">
            <div class="lk-charts-col">
                ...
                <div class="lk-chart-sub">
                    <div class="lk-chart-box" id="lk-echarts-pie" style="flex:1;"></div>
                    <div class="lk-chart-box" style="flex:2; padding:15px; display:flex; flex-direction:column; justify-content:center;">
                        <h4 style="margin:0 0 10px 0; color:#333;">💡 提示与说明</h4>
                        <ul style="font-size:13px; color:#555; padding-left:20px; line-height:1.6; margin:0;">
                            <li><b>外层时间轴</b> 用于在海量数据中圈定一个大的时间窗口。</li>
                            <li><b>内层拖拽轴 (下方)</b> 用于在当前图表视口内进行精密放大。</li>
                            <li>点击左侧 "开始录制"，可实时监控并锁定最新时间点，统计压测速率。</li>
                            <li>下拉框可进行<b>多维交叉筛选</b>（令牌、模型、分组等）。</li>
                        </ul>
                    </div>
                </div>
            </div>
            ...
            <div class="lk-side-col">
                ...
                <div class="lk-card">
                    <div class="lk-card-title">当前选区统计 (Inner View)</div>
                    <div class="lk-stat-val lk-stat-highlight" id="lk-v-total">$0.000</div>
                    <div class="lk-card-stat" style="margin-top:10px;"><span>Avg / Hour</span><span class="lk-stat-val" id="lk-v-avg-h">$0.00</span></div>
                    <div class="lk-card-stat" style="border:none; color:#888; font-size:11px;" id="lk-v-tokens">Tokens: 0 | Req: 0</div>
                </div>
            </div>
        </div>
    `;
    ...
    filterBar.appendChild(createMsUI('sel-group', '全部分组', 'group'));
    filterBar.appendChild(createMsUI('sel-type', '全部类型', 'type'));
    filterBar.appendChild(createMsUI('sel-model', '全部模型', 'model'));

    // 初始化 Echarts
    mainChartInstance = echarts.init(document.getElementById('lk-echarts-main'));
    pieChartInstance = echarts.init(document.getElementById('lk-echarts-pie'));
    ...
    ['lk-sel-scale', 'lk-sel-agg', 'lk-sel-chart', 'lk-sel-start', 'lk-sel-end'].forEach(id => {
        document.getElementById(id).onchange = updateChart;
    });

    window.addEventListener('resize', () => {
        if(panel.classList.contains('show')) { mainChartInstance.resize(); pieChartInstance.resize(); }
    });
